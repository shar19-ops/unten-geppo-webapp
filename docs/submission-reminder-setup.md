...
### 4. まず自分宛に1通だけ送って配信を確かめる

`dry_run` を **false**、`テスト送信の宛先` に**自分のメールアドレス**を入れて実行すると、対象が何台あっても**そのアドレスへ1通だけ**送ります（件名に `【テスト送信】` が付きます）。本来の宛先には送りません。

Power Automate から実際にメールが届けば、配信経路の確認は完了です。

### 5. 本番送信

`dry_run` を **false**、`テスト送信の宛先` を**空欄**にして実行すると、未提出の車両管理者全員へ送信されます。

> 対象は実在の社員です。件数はログの「送信対象 ○件」で事前に確認できます。通常は手動実行せず、**毎月3日の自動実行に任せてください。**
